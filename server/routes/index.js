...
var makeIssue = function () {
  ...
  // first_issue_date: 第一期开始时间（9:10）  end_issue_date: 最后一期结束时间

  var cur_issue, end_time, state; // 当前期，截止时间，状态值

  if (date.getTime() - first_issue_date > 0 && date.getTime() - end_issue_date < 0) {
    // 正常销售
    let cur_issue_date = new Date(); // 当期销售时间
    // 设置销售时间开始时间为 9:00:00
    ...
    // ceil() 方法执行的是向上取整
    var end_date = new Date(cur_issue_date.getTime() + 1000 * 60 * 10 * h); // 当期销售截止时间
    end_time = end_date.getTime();
    cur_issue = [end_date.getFullYear(), ('0' + (end_date.getMonth() + 1)).slice(-2), ('0' + end_date.getDate()).slice(-2), ('0' + h).slice(-2)].join('');
  } else {
    // 今天销售已截止
    if (data.getDate() != end_issue_date.getDate()) { // 如果当前时间不等于结束时间，则将时间调整为下一天
