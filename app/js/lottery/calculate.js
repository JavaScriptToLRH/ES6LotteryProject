...
class Calculate {
  ...
  computeCount (active, play_name) {
    ...
    const exist = this.play_list.has(play_name); // 判断玩法标识是否存在 play_list为玩法列表（即：任二、任三、任四、任五、任六、任七、任八）
    ...
    if (exist && play_name.at(0) === 'r') {
      ...
      count = Calculate.combine(arr, play_name.split('')[1]).length; // 进行组合运算
  ...
  computeBonus (active, play_name) {
    ...
    const play = play_name.split(''); // 获取当前玩法的一个基数，玩法分别为：r2、r3、r4、r5、r6、r7、r8,则获取玩法对应的数字（即：2/3/4/5/6/7/8）
